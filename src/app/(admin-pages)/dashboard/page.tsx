...
const AdminWorks = () => {
  ...
  const handleOptimisticSuccess = (workId: string) => {
    setWorks(prev =>
      prev.map(w =>
        w._id === workId
          ? { ...w, isOptimistic: false, originalData: undefined }
          : w
      )
    )
  }
    ...
    setWorks(prev =>
      prev.map(w =>
        w._id === workId && w.originalData
          ? { ...w, ...w.originalData, isOptimistic: false, originalData: undefined }
          : w
      )
    )
    setRecentlyUpdated(prev => {
              ...
              {works.map((work, index) => (
                <div
                  ...
                  className={cn(
                    ...
                    recentlyUpdated.has(work._id) && !work.isOptimistic && "border-green-400 shadow-green-200/50 shadow-lg",
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-500 font-medium">
                        Created {new Date(work.createdAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                      </p>
                      {work.isOptimistic && (
