...
const WorkSkeleton = () => (
  <div className="bg-white rounded-xl sm:rounded-2xl lg:rounded-3xl p-3 sm:p-4 lg:rounded-2xl lg:p-6 shadow-sm border border-gray-100 animate-pulse">
...
const AdminWorksPage = () => {
  ...
  const fetchMissingCategories = useCallback(async (worksData: Work[]) => {
    ...
    try {
      ...
      const newCategories = results.reduce((acc, result) => {
        if (result) acc[result.id] = result.name;
        return acc;
  ...
  const handleOptimisticSuccess = (workId: string) => {
    setWorks(prevWorks =>
      prevWorks.map(work =>
        work._id === workId
          ? { ...work, isOptimistic: false, originalData: undefined }
          : work
      )
    );
  };
    ...
    setWorks(prevWorks =>
      prevWorks.map(work =>
        work._id === workId && work.originalData
          ? { ...work, ...work.originalData, isOptimistic: false, originalData: undefined }
          : work
      )
    );
    setRecentlyUpdated(prev => {
  ...
  return (
    <div className="w-full min-h-screen h-full bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-blue-900 mb-20">
      <div className="w-full h-full py-4 sm:py-6 lg:py-10 px-3 sm:px-6 lg:px-8 xl:px-20 mb">
